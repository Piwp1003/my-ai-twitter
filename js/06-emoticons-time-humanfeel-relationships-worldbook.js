...
function getRelationshipContextPrompt(char) {
    ...
    const strangerRule = '\n【关于人物关系的重要规则】：下面列出的是你现在真实认识、有实际关系的人物，仅限这份名单。如果接下来的对话/情节里出现了不在这份名单里的角色——哪怕对方表现得很熟络、或者名字听起来眼熟——说明你们其实并不认识对方，你应该表现得像第一次见面的陌生人：保持礼貌但有距离感，不要用亲昵的称呼，不要表现出"我们很熟"的语气，也不要凭空编造过去的交集或互动历史，除非你的人设/世界书里明确写了你认识对方。\n';
    ...
    if (lines.length === 0) return strangerRule + '（你目前没有任何被记录下来的人物关系，所以理论上你现在谁都不认识。）\n';
