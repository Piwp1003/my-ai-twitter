...
(function () {
  ...
  let S = {
    ...
    watch: {
      chars: [],            // 正在一起看的角色 id
      sayOnScene: true,     // 每隔一段自己挑话说（主要开销）
      sceneMin: 5,          // 隔几分钟看一次这段字幕
      sayOnPause: true,     // 你一暂停 TA 接一句
      sayOnEnd: true,       // 看完给个感想
      danmu: true,          // 全屏时走弹幕
      danmuSpeed: 1,        // 弹幕速度倍率
      danmuSize: 15,        // 弹幕字号
      danmuOpacity: 0.92,
  ...
  async function save() { try { if (DB) await DB.setItem(KEY, S); } catch (e) { console.warn('[一起看] 存档失败', e); } }
  async function load() {
    try {
      if (!DB) return;
...
{"say": true或false, "quote": "戳到你的那一句原台词（没有就空字符串）", "text": "你想说的话，像看片时随口说的，不超过40字"}`;
  ...
  async function onPaused() {
    if (pauseBusy || !S.watch.sayOnPause) return;
...
说一句话，像真的在旁边被暂停打断了那样，不超过30字。只输出这句话本身。`;
      ...
      if (t) pushComment(c, t.replace(/^["「]|["」]$/g, '').slice(0, 100), '');
  ...
  async function onEnded() {
    const f = curFilm();
    if (!f) return;
    if (!S.watch.sayOnEnd) { toast('看完了', '《' + f.title + '》'); return; }
    const cs = watchers();
    for (const c of cs) {
  ...
  window.fbOpenWatchers = function () {
    ...
    modal('fbWatchersModal', `
      <div class="modal-box" style="width:400px;">
        <h2>👥 和谁一起看</h2>
        <div class="form-hint">可以选好几个。选了之后 TA 才会开口——没选人就是你一个人安静看片，一次 API 都不调。</div>
        <div class="fb-modal-body" style="margin:14px 0;">
          ${cs.length ? cs.map(c => `
            <label class="fb-row" style="cursor:pointer;">
              <input type="checkbox" ${(S.watch.chars || []).map(String).includes(String(c.id)) ? 'checked' : ''}
                     onchange="fbToggleWatcher('${c.id}', this.checked)" style="width:16px;height:16px;">
              <b style="flex:1;">${esc(c.name)}</b>
              <span style="font-size:11.5px;color:#8b98a5;">${esc((S.taste || {})[String(c.id)] || '')}</span>
              <button type="button" class="btn-edit-small" onclick="event.preventDefault();fbGenTaste('${c.id}')">口味</button>
            </label>`).join('') : '<div style="font-size:13px;color:#8b98a5;">还没有角色。</div>'}
        </div>
    ...
    modal('fbSetModal', `
      <div class="modal-box" style="width:430px;">
        ...
        <div class="fb-modal-body">
          ...
          <label class="fb-row" style="cursor:pointer;"><input type="checkbox" ${w.sayOnScene ? 'checked' : ''} onchange="fbSet('sayOnScene',this.checked)" style="width:16px;height:16px;">
          ...
          <label class="fb-row" style="cursor:pointer;"><input type="checkbox" ${w.sayOnPause ? 'checked' : ''} onchange="fbSet('sayOnPause',this.checked)" style="width:16px;height:16px;">
            <span style="flex:1;"><b>你一暂停 TA 接一句</b><br><span style="font-size:11.5px;color:#8b98a5;">每次暂停一次调用。</span></span></label>
          <label class="fb-row" style="cursor:pointer;"><input type="checkbox" ${w.sayOnEnd ? 'checked' : ''} onchange="fbSet('sayOnEnd',this.checked)" style="width:16px;height:16px;">
