...
async function callChatCompletionAPI(api, promptContent, maxRetries = 2) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            const res = await fetch(`${api.url}/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${api.key}` }, body: JSON.stringify({ model: api.model, messages: [{ role: "user", content: promptContent }] }) });
        ...
        return `<div class="wb-card" style="min-width:0; max-width:none; width:100%;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px;">
                <div style="flex:1; min-width:0;">
                    ...
                    <div style="font-size:12px; color:#8b98a5; margin-bottom:4px;">${scopeLabel}${p.description ? ' · ' + escapeHtml(p.description) : ''}</div>
                </div>
                <input type="checkbox" ${p.enabled !== false ? 'checked' : ''} onchange="togglePluginEnabled('${p.id}')" title="启用/禁用">
            </div>
            <div class="wb-content" style="font-family:monospace; font-size:11px;">${escapeHtml((p.promptText || p.actionPrompt || p.macroValue || p.code || '').slice(0, 200))}</div>
...
function createPlugin() {
    ...
    if (type === 'script') { p.code = document.getElementById('newPluginScriptCode').value; p.onResponse = document.getElementById('newPluginScriptOnResponse').value; }
    plugins.push(p);
    saveAllData();
    ['newPluginName', 'newPluginDesc', 'newPluginPromptText', 'newPluginActionLabel', 'newPluginActionPrompt', 'newPluginMacroName', 'newPluginMacroValue', 'newPluginScriptCode', 'newPluginScriptOnResponse'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
...
function renderMarkdownLite(text) {
    if (!text) return '';
    // 注意：代码块之外的内容故意不转义 HTML —— 因为正则脚本一个很常见的用法就是把AI输出的标签
    // 替换成自定义的HTML卡片样式（比如状态栏、时间跳跃提示），转义会让这些卡片变回一堆尖括号文字，
    // 直接破坏这个功能。这和原来（加markdown支持之前）网页本身不转义的行为也是一致的。
    // 但代码块（```...```或`...`）语义上就是"原样显示文字"，这里必须转义，否则一旦AI把HTML内容
    // 包进代码块里（比如```html\n<div>...</div>\n```），标签会被当成真HTML解析，还会把语言标识
    // "html"这几个字也一起显示出来。
    let t = String(text);
    // 代码块 ```lang\n...\n```：可选的语言标识（如html/js）要去掉，内容整体转义
    t = t.replace(/```[ \t]*([a-zA-Z0-9]*)[ \t]*\n?([\s\S]*?)```/g, (m, lang, code) => `<pre class="md-codeblock"><code>${escapeHtml(code.trim())}</code></pre>`);
    // 行内代码 `...`（同样需要转义）
    t = t.replace(/`([^`\n]+)`/g, (m, code) => `<code class="md-inlinecode">${escapeHtml(code)}</code>`);
    // 加粗 **text** / __text__
    t = t.replace(/\*\*([^\*\n]+)\*\*/g, '<strong>$1</strong>');
...
function restartChatWithGreeting(charId) {
    ...
    if (hasHistory) {
        const keepOld = confirm(`要保留现在和${char.name}的这段对话吗？\n【确定】= 自动生成一个分支角色保存旧对话，当前角色清空重新开始\n【取消】= 直接清空聊天记录重新开始（旧记录会丢失，无法找回）`);
        if (keepOld) {
            const newChar = JSON.parse(JSON.stringify(char));
            newChar.id = Date.now().toString();
            newChar.name = char.name + `（${new Date().toLocaleDateString('zh-CN')}之前）`;
            newChar.branchedFrom = char.id;
            newChar.branchedFromName = char.name;
            myCharacters.unshift(newChar);
            globalChats[newChar.id] = JSON.parse(JSON.stringify(globalChats[charId]));
            renderChatCharList();
        }
        globalChats[charId] = [];
    }
    ...
    if (options.length === 0) {
        ...
        alert(hasHistory ? '已经清空重新开始了（这个角色没有设置开场白，你可以先开口打个招呼）。' : '这个角色没有设置开场白，直接开口聊就行～');
