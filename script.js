...
async function sendProactiveChatMessage(char) {
    ...
    let prompt = `${buildBasePrompt(char, true)}
以下是你们最近的聊天记录：
${recentHistory || '(暂无记录)'}
现在，请结合你的性格，主动找用户开启一个新的话题，或者继续之前的话题。
直接输出你想发的话即可。你可以通过输出 [NUDGE] 主动拍一拍用户。
${emoPrompt}
字数${chatWordLimit}字以内。`;
    ...
    try {
        ...
        let repText = data.choices?.[0]?.message?.content?.trim() || "";
        if (!repText || (repText.toUpperCase().startsWith("NO") && repText.length < 5)) return;
        ...
        if (repText || repMediaUrl) {
            ...
            saveAllData(); 
            checkAndAutoSummarizeChat(sessionId);
        }
    } catch(e) { 
        console.error("主动发送消息网络请求失败:", e); 
    }
}
