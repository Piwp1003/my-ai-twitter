...
function selectEmoticon(url) {
    ...
    else if (emoticonPickerTarget === 'inline') { pendingInlineAttachments[param] = url; const pre = document.getElementById(`inlineAttachmentPreview-${emoticonPickerParam}`); pre.innerHTML = `<img src="${url}" class="emo-preview-img"><button class="emo-clear-btn" onclick="clearAttachment('inline', '${emoticonPickerParam}')">×</button>`; pre.style.display = 'block'; } 
...
function showFollowingContextMenu(e, charId) {
    e.preventDefault(); contextMenuTargetId = charId; const char = myCharacters.find(c => c.id === charId); if(!char) return;
...
async function postUserComment(postId) {
    ...
    // 核心修改：追加唯一的 id 用于支持楼中楼嵌套，parentId 为 null 代表一级评论
    const newReply = { 
        id: 'r_' + Date.now(), 
        parentId: null, 
        char: { ...currentUser }, 
    ...
    if (!newReply.likedBy.includes(char.id)) newReply.likedBy.push(char.id);
    ...
    for (let char of myCharacters) {
        ...
        let prompt = "";
        if (isPostAuthor) { 
            prompt = `${buildBasePrompt(char, true)}${emoPrompt}用户在你的帖子下评论了："${text}"。你必须与用户互动！如果你觉得无需长篇大论只需点赞，请直接输出"LIKE"；如果你想用文字回复，请直接输出回复的具体内容（不超过${postWordLimit}字）。绝对不能输出"NO"。若用表情包附上 [EMO:ID]。`; 
        } else { 
            prompt = char.persona.includes('高冷') ? `${buildBasePrompt(char, true)}用户在别人的帖子下评论："${text}"。是否点赞？输出"LIKE"或"NO"。` : `${buildBasePrompt(char, true)}${emoPrompt}用户在帖子下评论了："${text}"。是否回复（不超过${postWordLimit}字）？若用表情包附上 [EMO:ID]。不回复输出"NO"。`; 
        }
        try {
            ...
            if (repText.toUpperCase() === 'LIKE') { 
                ...
                // AI 回复用户时，将其挂载为刚才用户评论的子评论
                post.replies.push({ id: 'r_' + Date.now() + Math.floor(Math.random()*100), parentId: newReply.id, char: char, text: repText, timestamp: Date.now(), likes: 0, liked: false, replyTo: currentUser.name, mediaUrl: repMediaUrl }); 
                post.stats.comments++; 
    ...
    if (Math.random() < 0.4) spawnNpcComments(postId, false, { triggerName: currentUser.name, triggerText: text });
}
...
function renderFollowingList() {
    ...
    container.innerHTML = follows.map(char => `<div class=\"following-card\" oncontextmenu=\"showFollowingContextMenu(event, '${char.id}')\" onclick=\"switchMainView('profile', '${char.id}')\">${getAvatarHTML(char, 60)}<div style=\"font-weight:bold; font-size:15px; margin-bottom:5px;\">${char.name} ${char.verified ? verifiedSVG : ''} ${char.isSpecialFollow ? '<span class=\"special-star\"><svg class=\"blue-line-icon\" viewBox=\"0 0 24 24\" style=\"width:16px;height:16px;vertical-align:middle;margin-top:-2px;\"><polygon points=\"12 2 15 8 22 9 17 14 18 21 12 18 6 21 7 14 2 9 9 8 12 2\"></polygon></svg></span>' : ''}</div><div style=\"color:#536471; font-size:13px;\">${char.handle}</div>${char.group ? `<span style=\"display:inline-block; margin-top:4px; background:rgba(29,155,240,0.1); color:#1d9bf0; border-radius:9999px; font-size:11px; padding:1px 7px;\">${char.group}</span>` : ''}</div>`).join('');
}
...
async function spawnNpcComments(postId, postType, opts = {}) {
    const { triggerName, triggerText, isShura } = opts;
    let post;
...
${forumHint}
${context}
请模拟${count}个路人NPC看到后的${toneHint}评论，各不相同，每条不超过30字。请严格返回JSON数组: [{"name":"网友昵称","text":"评论内容"}]，不要有其他任何说明文字。`;
    ...
    try {
        ...
        arr.forEach(c => {
            ...
            if (postType === true || postType === 'tabloid') {
                post.replies.push({ charId: 'npc', name: c.name || '路人网友', text: c.text, timestamp: Date.now() });
                // 【新增：小报推文收到NPC回复的弹窗】
                ...
                post.replies.push({ charId: 'npc', anonName: c.name || '路人网友', anonId: 'NPC', text: c.text, timestamp: Date.now() });
                // 【新增：匿名推文收到NPC回复的弹窗】
...
async function submitInlineReply(postId, replyIdx) {
    ...
    else if (Math.random() < 0.4) spawnNpcComments(postId, false, { triggerName: currentUser.name, triggerText: text });
    ...
    const prompt = isCool 
        ? `你是"${char.name}"，人设：${char.persona}。${wbContext}${genderContext}${chatSummaryContext}${emoPrompt}用户在帖子下回复了你："${text}"。是否回复？直接输出内容或"NO"。若用表情包附上 [EMO:对应ID]。` 
        : `你是"${char.name}"，人设：${char.persona}。${wbContext}${genderContext}${chatSummaryContext}${emoPrompt}用户在帖子下@了你："${text}"。请回复（不超过${chatWordLimit}字）或输出"NO"。若想使用表情，请附带 [EMO:对应ID]。`;
    ...
    try {
        ...
        if (!repText.toUpperCase().startsWith("NO") && repText !== "") { 
            if(isTabloid) {
                post.replies.push({ id: 'r_' + Date.now(), parentId: newReply.id, charId: char.id, name: char.name, text: repText, timestamp: Date.now() });
            } else {
                post.replies.push({ id: 'r_' + Date.now(), parentId: newReply.id, char: char, text: repText, timestamp: Date.now(), likes: 0, liked: false, likedBy: [], mediaUrl: repMediaUrl, replyTo: currentUser.name }); 
            }
            post.stats.comments++; 
            addNotification(`<b>${char.name}</b> 回复了您`, postId, null, char, repText); 
            if (document.getElementById('view-post-detail').style.display !== 'none') renderSinglePostDetail(postId);
            saveAllData();
            if (isTabloid) { generateAutoNpcReaction(postId, char.name, repText); rollTabloidAIParticipation(postId, repText, char.name); }
            else if (Math.random() < 0.4) spawnNpcComments(postId, false, { triggerName: char.name, triggerText: repText });
        }
    } catch(e) {}
}
...
function generateAutoNpcReaction(postId, triggerName, triggerText) {
    spawnNpcComments(postId, true, { triggerName, triggerText });
}
...
async function userReplyTabloid(postId) {
    ...
    // 💡 写入用户暂存的表情数据
    post.replies.push({ charId: 'me', name: currentUser.name, text: text, timestamp: Date.now(), mediaUrl: pendingReplyAttachment });
    ...
    spawnNpcComments(postId, true, { triggerName: currentUser.name, triggerText: text });
    let mentioned = myCharacters.filter(c => text.includes('@' + c.name));
    ...
    if (mentioned.length > 0) {
        for(let char of mentioned) {
            ...
            try {
                ...
                let rep = (await res.json()).choices?.[0]?.message?.content?.trim();
                if(rep && !rep.toUpperCase().startsWith("NO")) {
                    post.replies.push({ charId: char.id, name: char.name, text: `回复 @${currentUser.name}：${rep}`, timestamp: Date.now() }); post.stats.comments++;
                    showToast(getAvatarHTML(char,40), `${char.name} 回复了你`, rep, post.id, null);
                    if (document.getElementById('view-post-detail').style.display !== 'none') renderSinglePostDetail(postId);
                    generateAutoNpcReaction(postId, char.name, rep);
...
// 2. 实现评论的点赞功能
function likeReply(replyIdx, event) {
    event.stopPropagation(); // 防止点击图标触发整条评论的点击事件
    
    // 找到当前正在查看的帖子
    // 假设我们有一个全局变量或者根据当前显示的 postId 获取
    // 这里我们直接通过 DOM 查找 postId，更加稳妥
    const parentReply = event.target.closest('.reply-item');
    const postId = parentReply ? parentReply.getAttribute('data-post-id') : null;
...
// ==========================================
// 11. 论坛专属新消息弹窗提醒器 (宽度加长与手机端适配)
// ==========================================
function showForumToast(title, content, avatarHtml) {
    ...
    container.style.right = '16px'; // 离右边留出呼吸空间
    ...
    toast.style.cssText = `
        ...
        max-width: 480px; /* 电脑端大概是详情页一半的长度 */
    ...
    setTimeout(() => { toast.style.transform = 'translateX(120%)'; setTimeout(() => toast.remove(), 5000); }, 20000);
}
