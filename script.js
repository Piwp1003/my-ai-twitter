...
function exportData() {
    const data = { myApiUrl, myApiKey, myModel, subApiUrl, subApiKey, subModel, currentUser, myCharacters, trendingTags, globalPosts, anonPosts, globalNotifications, characterGroups, factionColors, charRelationships, relationshipTypePresets, statusTypes, worldbooks, globalEmoticons, globalChats, groupChats, globalBgImage, globalBgOpacity, allowActionTags, enableScheduleAutoCheck, enableScheduleChatLink, enableAffinitySystem, enableCharBackgroundInteraction, enableTypingIndicator, enableAnniversary, memoryAlbum, chatWordLimit, postWordLimit, diaryWordLimit, letterWordLimit, globalNovels, novelCustomCSS, globalCustomCSS, tabloidPosts, siteLogoImg, forumThreads };
...
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            ['myApiUrl','myApiKey','myModel','subApiUrl','subApiKey','subModel','trendingTags','globalPosts','anonPosts','globalNotifications','characterGroups','factionColors','charRelationships','relationshipTypePresets','statusTypes','worldbooks','globalEmoticons','globalChats','groupChats','globalBgImage','globalBgOpacity','allowActionTags','enableScheduleAutoCheck','enableScheduleChatLink','enableAffinitySystem','enableCharBackgroundInteraction','enableTypingIndicator','enableAnniversary','memoryAlbum','chatWordLimit','postWordLimit','diaryWordLimit','letterWordLimit','globalNovels','novelCustomCSS','globalCustomCSS','tabloidPosts','siteLogoImg','forumThreads'].forEach(k => { if(data[k] !== undefined) eval(`${k} = data[k];`); });
            ...
            updateUserMiniProfile(); if(typeof renderTrends === 'function') renderTrends(); updateGlobalBgStyles(); applyGlobalCSS(); applyNovelCSS(); updateSiteLogo(); switchMainView('home'); updateCharSelects(); alert("数据恢复成功！欢迎回来。"); saveAllData();
...
function openCharCalendarModal(charId) {
    ...
    const anniEl = document.getElementById('charCalendarAnniversary');
    if (!history || history.length === 0) {
        anniEl.innerHTML = '还没有和ta聊过天，暂时没有纪念日记录～';
    } else {
        const firstTs = history[0].timestamp;
        const daysSince = Math.floor((Date.now() - firstTs) / 86400000);
        anniEl.innerHTML = `认识于 ${new Date(firstTs).toLocaleDateString('zh-CN')}，到今天已经 <b style="color:#1d9bf0;">${daysSince}</b> 天了。`;
    }

    const memories = memoryAlbum.filter(m => m.charId == charId);
    const memEl = document.getElementById('charCalendarMemoryList');
    if (memories.length === 0) {
        memEl.innerHTML = '<div class="empty-state" style="padding:15px;">还没有收藏和ta有关的回忆～</div>';
    } else {
        memEl.innerHTML = memories.map(m => `
            <div class="memory-card" style="margin:0 0 10px;" onclick="closeModal('charCalendarModal'); jumpToMemorySource(${JSON.stringify(m).replace(/"/g, '&quot;')})">
                <div style="font-size:12px; color:#8b98a5;">${m.type === 'post' ? '推文' : '聊天'} · ${new Date(m.timestamp).toLocaleDateString('zh-CN')}</div>
...
function renderChatCharList() {
    ...
    [...groupChats, ...myCharacters].forEach(x => {
        ...
        html += `<div class="chat-char-item ${currentChatSessionId == x.id ? 'active' : ''}" onclick="switchChatSession('${x.id}')">
            <div style="position:relative; display:inline-block;" onclick="openChatOptions('${x.id}', event)" ${isGroupItem ? '' : `oncontextmenu="showAvatarContextMenu(event, '${x.id}')"`}>${x.members ? getGroupAvatarHTML(x, 50) : getAvatarHTML(x, 50)}${unreadHtml}</div>
...
function switchChatSession(id) {
    ...
    // 自动刷新该角色状态（生成/更新日程）
    if(!id.toString().startsWith('g_')) {
        const char = myCharacters.find(c => c.id == id.toString());
        if(char && !char.schedule) {
            runScheduleGeneration(id.toString());
        }
    }
}
...
async function triggerAIBatchReply(sessionId, triggerText) {
    ...
    for (let char of targetChars) {
        ...
        let replyRule = isMentioned ? `你被艾特了（或这是私聊），你【必须】回复，不能输出"NO"。` : (isGroup ? `如果觉得群里没人理你且无需回复，直接输出"NO"。` : `如果不知道怎么回可以输出"NO"。`);
        
        // 核心改造：引入多段真实聊天模拟提示词
        let prompt = `${buildBasePrompt(char, true)}${getTimeAwarenessPrompt(sessionId, char)}${getChatNaturalnessPrompt()}\n以下是我们${isGroup?'群里':''}最近的聊天记录：\n${recentHistory}
...
- 额外任务（不外显）：请在返回的 JSON 里加一个 "stateUpdate" 字段，写一句20字以内的话，描述"回复完这轮之后你大概会去做什么/处于什么状态"，仅用于系统内部记录、给你自己下一次的状态延续做参考，绝不会展示给用户，所以不需要考虑对话自然度，纯粹是内部备忘。${statusTypes.length > 0 ? `再加一个 "statusTypeLabel" 字段，从这些标签里选一个最贴近你此刻状态的：[${statusTypes.map(t => t.label).join('、')}]，如果都不太贴切就留空字符串。` : ''}${affinityInstruction}`;
}
...
function getCharacterWorldbookText(char) {
    ...
    // 按权重从高到低排序：权重高的世界书排在前面、优先被阅读，权重低的排在最后
    combined.sort((a, b) => (b.weight ?? 50) - (a.weight ?? 50));
    
    if (combined.length === 0) return '';
    return `【必须强制结合的世界书背景 — 已按权重从高到低排列，权重越高的设定优先级越高、越要优先遵循】：\n` + combined.map(w => `- [权重${w.weight ?? 50}] ${w.title}: ${w.content}`).join('\n') + `\n你在生成任何内容时都必须严格基于上述世界观；如果不同设定之间出现冲突，以权重更高的为准。\n\n`;
}

// ===== 关系网驱动的角色互动：推文/爆料/论坛提及时，让有关系的角色自己决定要不要来 =====
async function triggerRelatedCharacterReactions(sourceChar, contextText, target) {
    if (!enableCharBackgroundInteractionPassive && !enableCharBackgroundInteractionActive) return; // 未开启任何互动层级时，不主动触发关系网联动
...
function saveWorldbook() {
    ...
    if (id) {
        const wb = worldbooks.find(w => w.id == id);
        if (wb) { wb.title = title; wb.content = content; wb.weight = weight; } // 保留原来的 isGlobal 状态
    } else {
        // 新增世界书默认不全局生效
        worldbooks.push({ id: Date.now(), title, content, isGlobal: false, weight });
    }
...
function startProactiveChatTimer() {
    ...
    // 角色背着用户私下互动：低频率检查，只在开关打开时生效
    setInterval(checkBackgroundCharInteractions, 5 * 60000);
}

async function checkBackgroundCharInteractions() {
    if (!enableCharBackgroundInteractionPassive && !enableCharBackgroundInteractionActive) return; // 未开启任何互动层级
    if (!charRelationships || charRelationships.length === 0) return;
    if (myCharacters.length < 2) return;
    if (Math.random() > 0.15) return; // 低概率触发，避免刷屏和过多API调用

    const api = getApiConfig(true); if (!api.key) return;
    const charEdges = charRelationships.filter(r => r.fromId !== 'me' && r.toId !== 'me');
    if (charEdges.length === 0) return;
    const edge = charEdges[Math.floor(Math.random() * charEdges.length)];
    const charA = myCharacters.find(c => c.id == edge.fromId);
    const charB = myCharacters.find(c => c.id == edge.toId);
    if (!charA || !charB) return;

    try {
        const prompt = `你正在为两个角色生成一件"背着用户悄悄发生"的小事，不会展示给用户看，只用来记录他们各自的生活状态。
角色A："${charA.name}"，人设：${charA.persona}。
角色B："${charB.name}"，人设：${charB.persona}。
两人关系：${edge.label}。
请设想一件符合双方人设和关系的简短互动或经历（比如一起吃了顿饭、吵了一架、互相发消息聊了什么、偶遇等），分别用一句话描述这件事之后A和B各自此刻的状态。
严格按JSON格式输出，不要markdown、不要多余说明：
{"eventForA": "20字以内，A此刻的状态", "eventForB": "20字以内，B此刻的状态"}`;
        const res = await fetch(`${api.url}/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${api.key}` }, body: JSON.stringify({ model: api.model, messages: [{ role: "user", content: prompt }] }) });
        const data = await res.json();
        ...
        rawText = rawText.replace(/^```json/i, '').replace(/^```/i, '').replace(/```$/i, '').trim();
        const parsed = JSON.parse(rawText);
        if (parsed.eventForA) saveCharLifeState(charA, parsed.eventForA);
        if (parsed.eventForB) saveCharLifeState(charB, parsed.eventForB);
        saveAllData();
    } catch (e) { console.error('角色背后互动生成失败:', e); }
}
...
async function sendProactiveChatMessage(char) {
    ...
    let recentHistory = globalChats[sessionId].slice(-10).map(m => m.sender === 'system' ? m.text : `${m.sender === 'me' ? currentUser.name : char.name}: ${m.text}`).join('\n');
    let emoPrompt = getEmoticonPrompt();
    
    let prompt = `${buildBasePrompt(char, true)}${getTimeAwarenessPrompt(sessionId, char)}${getChatNaturalnessPrompt()}
以下是你们最近的聊天记录：
${recentHistory || '(暂无记录)'}
现在，请结合你的性格，主动找用户开启一个新的话题，或者继续之前的话题。
...
function hideAllViews() {
    ['home','profile','tag','search','notifications','post-detail','following-list','chat','anon-forum','diary','novel','tabloid','mobile-trends','faction-network','faction-members','char-relations','faction-overview','memory-album','character-center'].forEach(v => { const el = document.getElementById(`view-${v}`); if(el) el.style.display = 'none'; });
...
const mobileViewTitles = { home:'主页', anonForum:'匿名论坛', diary:'信件与日记', novel:'故事', tabloid:'营销号', profile:'个人资料', tag:'标签', search:'全站搜索', notifications:'通知', postDetail:'帖子', followingList:'我的关注', chat:'聊天', factionNetwork:'势力关系网', factionMembers:'势力成员', charRelations:'角色关系', factionOverview:'势力总览', memoryAlbum:'回忆相册', characterCenter:'角色中心', settings:'系统设置', worldbook:'世界书' };
...
function switchMainView(viewId, param = null) {
    ...
    else if (viewId === 'memoryAlbum') { document.getElementById('view-memory-album').style.display = 'block'; renderMemoryAlbum(); }
...
function toggleMemoryStar(event, type, refId) {
    ...
    saveAllData();
    if (document.getElementById('view-home') && document.getElementById('view-home').style.display !== 'none') renderPosts();
    if (document.getElementById('view-post-detail') && document.getElementById('view-post-detail').style.display !== 'none' && typeof renderSinglePostDetail === 'function') renderSinglePostDetail(refId);
    if (document.getElementById('view-memory-album') && document.getElementById('view-memory-album').style.display !== 'none') renderMemoryAlbum();
}
...
function renderMemoryAlbum() {
    const container = document.getElementById('memoryAlbumList');
    if (memoryAlbum.length === 0) { container.innerHTML = '<div class="empty-state">还没有收藏任何回忆～聊天消息右键、推文点☆号都可以收藏哦。</div>'; return; }
    container.innerHTML = memoryAlbum.map(m => {
...
function openDiaryFromSearch(charId, kind, itemId) {
    ...
    currentDiaryTab = kind === 'letters' ? 'letter' : 'diary';
    switchMainView('profile', charId);
...
function clearForm() {
    ...
    ['charName', 'charHandle', 'charPersona', 'charBio', 'charFollowers', 'charFollowing', 'charLocation', 'charWebsite', 'charBirthdate', 'charAutoReply', 'charBusyAutoReply', 'charAnonName', 'charAnonId', 'charNudgeText'].forEach(id => document.getElementById(id).value = '');
    ...
    if (edges.length === 0) {
        ...
        document.getElementById('relationsListArea').innerHTML = '<div style="font-weight:bold; margin-bottom:8px; color:#536471;">关系列表</div>' + edges.map(r => {
            ...
            return `<div class="rel-chip" style="border-color:${r.color};">
                ...
                <span class="rel-chip-del" onclick="deleteRelationship('${r.id}')">×</span>
            </div>`;
        }).join('');
    }
}
