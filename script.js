...
function applyRegexScripts(text, target) {
    ...
    regexScripts.filter(s => s.enabled !== false && (s.target === target || s.target === 'both')).forEach(s => {
        ...
        try {
            if (s.isRegex) {
                const re = new RegExp(s.find, s.flags || 'g');
...
async function getSemanticContext(sessionId, char, queryText) {
    ...
    if (enableVectorMemory) {
        ...
        const recentTimestamps = new Set(history.slice(-10).map(m => m.timestamp));
    ...
    return memoryPart + dataBankPart;
}
...
function saveAllData() {
    const dataToSave = {
        ...
        regexScripts, enableVectorMemory, embeddingModel, dataBank // <--- 新增：正则脚本、向量记忆设置、角色资料库
    };
        ...
        return `
            <div class="chat-msg-row ${isMe ? 'me' : 'other'}">
                ...
                <div class="chat-bubble-wrapper" style="align-items: ${isMe ? 'flex-end' : 'flex-start'};">
                    ...
                    <div class="chat-bubble ${isMe ? 'me' : 'other'}" oncontextmenu="showChatContextMenu(event, ${idx})" ontouchstart="chatBubbleTouchStart(event, ${idx})" ontouchend="chatBubbleTouchEnd(event)" ontouchmove="chatBubbleTouchEnd(event)">${msg.quote ? `<div class="chat-quote-bubble"><b>${msg.quote.name}</b>: ${msg.quote.text}</div>` : ''}${msg.text}${msg.mediaUrl ? `<img src="${msg.mediaUrl}">` : ''}${swipeHtml}</div>
                    ${isMe ? readStatusHtml : ''}
                </div>
                ${isMe ? avatarHtml : ''}
            </div>`;
...
window.contextActionRegenerateChat = async function() {
    ...
    const recentHistory = historyForPrompt.slice(-10).map(m => m.sender === 'system' ? m.text : `${m.sender === 'me' ? currentUser.name : (myCharacters.find(c=>c.id==m.sender)?.name || '未知')}: ${m.quote ? `[回复 ${m.quote.name}: ${m.quote.text}] ` : ''}${m.text}`).join('\n');
...
function getCharacterWorldbookText(char, chatHistoryStr = "") {
    ...
    return `【当前触发的世界观设定（请严格遵循）】：\n` + finalWbs.map(w => `- [权重${w.weight ?? 50}] ${w.title}: ${w.content}`).join('\n') + `\n\n`;
}
...
function buildBasePrompt(char, includeChatSummary = true, chatHistoryStr = "") {
    ...
    prompt += `\n【格式规则】：${actionRule}\n`;
    return applyMacros(prompt, char);
}
...
const mobileViewTitles = { home:'主页', anonForum:'匿名论坛', diary:'信件与日记', novel:'故事', tabloid:'营销号', profile:'个人资料', tag:'标签', search:'全站搜索', notifications:'通知', postDetail:'帖子', followingList:'我的关注', chat:'聊天', factionNetwork:'势力关系网', factionMembers:'势力成员', charRelations:'角色关系', factionOverview:'势力总览', characterCenter:'角色中心', settings:'系统设置', worldbook:'世界书' };
...
{"bio": "提取的短简介", "anonName": "提取的昵称", "nudgeText": "拍一拍文本"}`;
    ...
    // 6. 智能侦测并挂载【世界书 Lorebook】
    if (data.character_book && data.character_book.entries && data.character_book.entries.length > 0) {
        if (confirm(`🎉 角色读取成功！\n系统检测到该角色卡内嵌了 ${data.character_book.entries.length} 条世界观设定(Lorebook)。\n是否自动将其导入到谷雨的世界书中并为其绑定？`)) {
            
            let wbText = data.character_book.entries.map(e => `[词条关键词: ${(e.keys||[]).join(', ')}]\n${e.content}`).join('\n\n');
            const wbId = Date.now();
            
            worldbooks.push({
                id: wbId,
                title: `${data.name || '导入角色'} 的专属世界设定`,
                content: wbText.substring(0, 5000), // 防止字数爆炸卡死
                isGlobal: false, // 专属世界书，不污染全局
                weight: 50
            });
            saveAllData();
            
            // 自动在多选框里把这本新诞生的世界书勾选上
            setTimeout(() => {
                const wbBox = document.getElementById('charWbCheckboxes');
                if (wbBox) {
                    wbBox.innerHTML += `<label style="display:flex; align-items:center; gap:4px; font-size:13px; color:#f91880; font-weight:bold;"><input type="checkbox" value="${wbId}" class="wb-check" checked> ${data.name || '导入角色'} 的专属世界设定</label>`;
                }
            }, 300);
            
            alert("世界书提取成功并已自动绑定！请浏览下方表格，没问题后点击最底部的【保存并生成角色】即可。");
            return;
        }
    }
...
// ==========================================
// 🧩 简易插件管理系统 (Plugin System)
// ==========================================
window.MyPluginSystem = {
    // 从本地浏览器的存储中读取已安装的插件
    plugins: JSON.parse(localStorage.getItem('my_custom_plugins') || '[]'),
    
    // 初始化：网页一打开，就悄悄运行所有亮绿灯的插件
    init: function() {
        this.plugins.forEach(plugin => {
            if(plugin.enabled) {
                try {
                    eval(plugin.code); // eval 可以将字符串作为 JS 代码执行
                    console.log(`✅ 插件 [${plugin.name}] 运行成功`);
                } catch(e) {
                    console.error(`❌ 插件 [${plugin.name}] 运行报错:`, e);
                }
            }
        });
    },
    ...
    // 添加并保存新插件
    addPlugin: function(name, code) {
        this.plugins.push({ id: Date.now(), name: name, code: code, enabled: true });
        this.save();
    },
    
    save: function() {
        localStorage.setItem('my_custom_plugins', JSON.stringify(this.plugins));
    }
};

// 页面加载完毕后，延迟一秒执行插件（防止插件找不到还没加载完的网页元素）
setTimeout(() => {
    window.MyPluginSystem.init();
}, 1000);
...
function renderPluginList() {
    ...
    if (window.MyPluginSystem.plugins.length === 0) {
        list.innerHTML = '<div style="color: #8b98a5; text-align: center; padding: 10px;">暂无安装的插件，快去下方粘贴代码安装一个吧！</div>';
        return;
    }
    list.innerHTML = window.MyPluginSystem.plugins.map((p, index) => `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px dashed #eff3f4;">
            <div style="font-weight: bold; color: ${p.enabled ? '#0f1419' : '#8b98a5'};">
                ${p.name} ${p.enabled ? '🟢' : '⚪'}
            </div>
            <div style="display: flex; gap: 5px;">
                <button class="btn-edit-small" onclick="togglePlugin(${index})">${p.enabled ? '停用' : '启用'}</button>
                <button class="btn-edit-small" style="color: #f91880; border-color: #f91880;" onclick="deletePlugin(${index})">删除</button>
            </div>
        </div>
    `).join('');
}
...
function installPlugin() {
    ...
    const code = document.getElementById('newPluginCode').value.trim();
    if (!name || !code) return alert('插件名称和代码都不能为空哦！');
    
    window.MyPluginSystem.addPlugin(name, code);
    document.getElementById('newPluginName').value = '';
    document.getElementById('newPluginCode').value = '';
    alert('🎉 安装成功！刷新网页后插件就会生效。');
    renderPluginList();
}

function togglePlugin(index) {
    let p = window.MyPluginSystem.plugins[index];
    p.enabled = !p.enabled;
    window.MyPluginSystem.save();
    renderPluginList();
    alert(p.enabled ? '已启用，刷新网页后生效！' : '已停用，刷新网页后恢复原样！');
}

function deletePlugin(index) {
    if(confirm('确定要彻底删除这个插件吗？')) {
        window.MyPluginSystem.plugins.splice(index, 1);
        window.MyPluginSystem.save();
        renderPluginList();
    }
}