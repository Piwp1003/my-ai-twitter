...
这是你自己发的帖子，有人评论了就必须回应，不能装作没看见。直接输出你的回复内容（不超过${chatWordLimit}字，语气符合匿名论坛的恶劣氛围和你的隐藏性格），绝对不能输出"NO"。${actionStrictRule}`;
                    const res = await fetch(`${api.url}/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${api.key}` }, body: JSON.stringify({ model: api.model, messages: [{ role: "user", content: p }] }) });
                    const data = await res.json();
