...
function saveAllData() {
    const dataToSave = {
        ...
        myCharacters, globalPosts, anonPosts, characterGroups, factionColors, charRelationships, relationshipTypePresets, globalEmoticons, worldbooks, globalChats, groupChats, currentUser, tabloidAccount, trendingTags,
...
function exportData() {
    const data = { myApiUrl, myApiKey, myModel, subApiUrl, subApiKey, subModel, currentUser, myCharacters, trendingTags, globalPosts, anonPosts, globalNotifications, characterGroups, factionColors, charRelationships, relationshipTypePresets, worldbooks, globalEmoticons, globalChats, groupChats, globalBgImage, globalBgOpacity, allowActionTags, chatWordLimit, postWordLimit, diaryWordLimit, letterWordLimit, globalNovels, novelCustomCSS, globalCustomCSS, tabloidPosts, siteLogoImg };
...
async function generateCharSchedule(charId) {
...
{"schedule": "从早到晚的完整日程，按时间段分行，用\\n分隔", "currentStatus": "结合日程和现在的真实时间，你此刻正在做的事，20字以内，不含引号"}`;
    ...
    try {
        ...
        if (parsed.currentStatus) saveCharLifeState(char, parsed.currentStatus);
        saveAllData();
        alert(`已为 ${char.name} 生成今日日程！点击ta的头像即可查看当前状态和完整日程。`);
    } catch (e) { alert('生成日程失败：' + e.message); }
}
...
function showCharLifeStatePopup(charId, event) {
    ...
    if (char.lifeState && char.lifeState.activity) {
        ...
        textEl.innerHTML = `💭 ${char.lifeState.activity}<br><span style="font-size:11px; color:#8b98a5;">（${ago}前）</span>`;
    } else {
        textEl.innerHTML = `暂时还不知道ta在做什么，多聊聊看吧～`;
    }
...
function toggleScheduleInBubble() {
    ...
    const expanded = scheduleText.dataset.expanded === '1';
    if (expanded) {
        scheduleText.style.display = 'none'; scheduleText.dataset.expanded = '0';
        scheduleToggle.innerText = '📅 查看今日日程';
    } else {
        scheduleText.innerText = char.schedule.text;
        scheduleText.style.display = 'block'; scheduleText.dataset.expanded = '1';
        scheduleToggle.innerText = '📅 收起日程';
    }
}
...
function renderChatMessages() {
    ...
    container.innerHTML = history.map((msg, idx) => {
        ...
        let avatarHtml = !isMe && senderChar ? `<div style="cursor:pointer;" onclick="showCharLifeStatePopup('${senderChar.id}', event)" ondblclick="triggerNudge('${currentChatSessionId}', '${senderChar.id}')" title="点击查看状态·双击拍一拍">${getAvatarHTML(senderChar, 40)}</div>` : `<div style="cursor:pointer;" ondblclick="triggerNudge('${currentChatSessionId}', 'me')" title="双击拍一拍">${getAvatarHTML(currentUser, 40)}</div>`;
        return `
  ...
  "stateUpdate": "打算回去继续睡回笼觉"
        ...
        try {
            ...
            try {
                ...
                if (parsed.stateUpdate) saveCharLifeState(char, parsed.stateUpdate);
            } catch (err) {
...
- 现在的真实时间：${nowStr}；距离你们上一次互动，实际已经过去约 ${elapsedText}。${lifeStateLine}
...
- 额外任务（不外显）：请在返回的 JSON 里加一个 "stateUpdate" 字段，写一句20字以内的话，描述"回复完这轮之后你大概会去做什么/处于什么状态"，仅用于系统内部记录、给你自己下一次的状态延续做参考，绝不会展示给用户，所以不需要考虑对话自然度，纯粹是内部备忘。`;
}
...
function saveCharLifeState(char, stateUpdate) {
    if (!char || !stateUpdate || typeof stateUpdate !== 'string') return;
    const clean = stateUpdate.trim().slice(0, 60);
    if (!clean) return;
    char.lifeState = { activity: clean, updatedAt: Date.now() };
}
...
function hideAllViews() {
    ['home','profile','tag','search','notifications','post-detail','following-list','chat','anon-forum','diary','novel','tabloid','mobile-trends','faction-network','faction-members','char-relations','faction-overview'].forEach(v => { const el = document.getElementById(`view-${v}`); if(el) el.style.display = 'none'; });
...
const mobileViewTitles = { home:'主页', anonForum:'匿名论坛', diary:'信件与日记', novel:'故事', tabloid:'营销号', profile:'个人资料', tag:'标签', search:'搜索', notifications:'通知', postDetail:'帖子', followingList:'我的关注', chat:'聊天', factionNetwork:'势力关系网', factionMembers:'势力成员', charRelations:'角色关系', factionOverview:'势力总览' };
...
function renderCharRelationsView(charId) {
    ...
    relatedChars.forEach((item, i) => {
        ...
        nodesHtml += `<div class="relation-node" style="left:${nx}px; top:${ny}px;" onclick="${nodeClick}">${getAvatarHTML(item.char, 56)}<div class="relation-node-name">${item.char.name}</div></div>`;
        // 箭头线：从中心指向对方
        ...
        linesHtml += `<line x1="${sx}" y1="${sy}" x2="${ex}" y2="${ey}" stroke="${color}" stroke-width="2.5" marker-end="url(#relArrow${i})"></line>
