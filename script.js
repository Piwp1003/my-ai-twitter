...
async function deleteWbCategory(idx) {
    ...
    if (!(await appConfirm(`删除分类"${catName}"？该分类下的世界书将变为无分类。`))) return;
...
发一条推文，分享你的见闻或看法，不超过${finalWordLimit}字（这段纯文字，不要在里面写"#标签"，标签单独填在下面JSON的tag字段里）。
【tag字段怎么填，按顺序判断】：
...
{"text":"你的推文正文（不含#标签）","tag":"符合上面规则就填'#标签'字符串，不符合就填null","image":false,"location":"当前所在的具体地点，若没有可填空字符串","emoticonId":"如果有合适的可用表情包，可以填入其ID，若不需要表情包则填null"}
如果内容非常适合真实配图，可以返回：
{"text":"推文正文（不含#标签）","tag":"同上规则","image":true,"imageKeyword":"配图关键词英文","location":"具体地点","emoticonId":null}`;
...
{"bio": "提取的短简介", "anonName": "提取的昵称", "nudgeText": "拍一拍文本"}`;
    ...
    if (Array.isArray(embeddedRegex) && embeddedRegex.length > 0) {
        if (await appConfirm(`🎉 还检测到该角色卡内嵌了 ${embeddedRegex.length} 条正则脚本。\n是否一并导入？`)) {
