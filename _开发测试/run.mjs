...
check('新建后进入工作台', await page.evaluate(() =>
  document.getElementById('storyStudioWorkView').style.display === 'flex'));
check('会话已入库', await page.evaluate(() => storySessions.length === 1));
check('默认停在续写页', await page.evaluate(() =>
  document.getElementById('ssPane-write').style.display === 'flex'));
...
check('楼层号从 0 开始（酒馆语义）', (await page.locator('#ssTurns .ss-meta').first().textContent()).includes('#0楼'));
...
await page.click('#ssActions button:has-text("重roll")');
...
// ---------- 13. 章节存档 ----------
await page.click('#ssActions button:has-text("存为新章节")');
await page.waitForTimeout(500);
...
check('输入框在剧情区下方（钉在最底下）', await page.evaluate(() => {
  const pane = document.getElementById('ssPane-write');
  const kids = Array.from(pane.children).map(e => e.id || e.className);
  const turns = kids.findIndex(k => k === 'ssTurns');
  const input = kids.findIndex(k => String(k).includes('chat-input-area'));
  return turns > -1 && input > turns && input === kids.length - 1;
}));
